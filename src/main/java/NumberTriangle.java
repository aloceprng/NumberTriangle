...
public class NumberTriangle {
    ...
    public static NumberTriangle loadTriangle(String fname) throws IOException {
        ...
        ArrayList<NumberTriangle[]> triangle = new ArrayList<>();

        while (line != null) {
            String[] rootValues = line.split("\\s+");
            ArrayList <NumberTriangle> row =  new ArrayList<>();
            for (int i = 0; i < rootValues.length; i++) {
                NumberTriangle value = new NumberTriangle(Integer.parseInt(rootValues[i]));
                row.add(value);
            }
            triangle.add(row.toArray(new NumberTriangle[0]));
            line = br.readLine();
        }
        br.close();

        for (int j = 0; j < triangle.size() - 1; j++) {
            NumberTriangle[] thisRow = triangle.get(j);
            NumberTriangle[] nextRow = triangle.get(j + 1);
            for (int k = 0; k < thisRow.length; k++) {
                thisRow[k].left = nextRow[k];
                thisRow[k].right = nextRow[k + 1];
            }
        }
